...
#         if A_egrmax != None:
#             self.A_egrmax = torch.nn.Parameter(torch.tensor([1.0]), requires_grad=True)
#         else: self.A_egrmax = None

#         if c_omega_psi_1 != None:
#             self.c_omega_psi_1 = torch.nn.Parameter(torch.tensor([1e-1]), requires_grad=True)
#         else: self.c_omega_psi_1 = None

#         if c_omega_psi_2 != None:
#             self.c_omega_psi_2 = torch.nn.Parameter(torch.tensor([1e-1]), requires_grad=True)
#         else: self.c_omega_psi_2 = None

#         if c_omega_psi_3 != None:
#             self.c_omega_psi_3 = torch.nn.Parameter(torch.tensor([1e-1]), requires_grad=True)
#         else: self.c_omega_psi_3 = None

#         if c_omega_phi_1 != None:
#             self.c_omega_phi_1 = torch.nn.Parameter(torch.tensor([1e-1]), requires_grad=True)
#         else: self.c_omega_phi_1 = None
        
#         if c_omega_phi_2 != None:
#             self.c_omega_phi_2 = torch.nn.Parameter(torch.tensor([1e-1]), requires_grad=True)
#         else: self.c_omega_phi_2 = None
        
#         if c_omega_phi_3 != None:
#             self.c_omega_phi_3 = torch.nn.Parameter(torch.tensor([1e-1]), requires_grad=True)
#         else: self.c_omega_phi_3 = None
...
#         ret_list = []
#         ret_list.append(self.net(x_norm))
        
#         # use softplus to make sure trainable variable is positive
#         if self.A_egrmax != None:
#             self.A_egrmax.data = softplus(self.A_egrmax)
#             ret_list.append(self.A_egrmax)
#         if self.c_omega_psi_1 != None:
#             self.c_omega_psi_1.data = softplus(self.c_omega_psi_1)
#             ret_list.append(self.c_omega_psi_1)
#         if self.c_omega_psi_2 != None:
#             self.c_omega_psi_2.data = -softplus(self.c_omega_psi_2)
#             ret_list.append(self.c_omega_psi_2)
#         if self.c_omega_psi_3 != None:
#             self.c_omega_psi_3.data = softplus(self.c_omega_psi_3)
#             ret_list.append(self.c_omega_psi_3)
#         if self.c_omega_phi_1 != None:
#             self.c_omega_phi_1.data = -softplus(self.c_omega_phi_1)
#             ret_list.append(self.c_omega_phi_1)
#         if self.c_omega_phi_2 != None:
#             self.c_omega_phi_2.data = -softplus(self.c_omega_phi_2)
#             ret_list.append(self.c_omega_phi_2)
#         if self.c_omega_phi_3 != None:
#             self.c_omega_phi_3.data = softplus(self.c_omega_phi_3)
#             ret_list.append(self.c_omega_phi_3)
...
# # time mapping  [0 30] -> [-1 1]
# # def time_mapping(t_tensor):
# #     t_min = torch.min(t_tensor)
# #     t_max = torch.max(t_tensor)
# #     t_tensor_ret = -1 + (2*(t_tensor - t_min) / (t_max - t_min))
# #     return t_tensor_ret
...
# omega_t_gt   = torch.from_numpy(omega_t ).float().to(device)
...
# num_neurons = 10
# output_dim = 2
# pim_pem_nn = PINN(input_dim, hidden_layers, num_neurons, output_dim, 
#                   A_egrmax=1).to(device)
...
# num_neurons = 10
# output_dim = 1
# omega_t_nn = PINN(input_dim, hidden_layers, num_neurons, output_dim,
#                   c_omega_psi_1 = 1,
#                  c_omega_psi_2 = 1,
#                  c_omega_psi_3 = 1,
#                  c_omega_phi_1 = 1,
#                  c_omega_phi_2 = 1,
#                  c_omega_phi_3 = 1).to(device)
...
# lambda_p_im_data     = torch.nn.Parameter(torch.full((t_tensor.shape[0],1), 1000.0)).detach().to(device).requires_grad_(True)
# lambda_p_em_data     = torch.nn.Parameter(torch.full((t_tensor.shape[0],1), 1000.0)).detach().to(device).requires_grad_(True)
# lambda_omega_t_data  = torch.nn.Parameter(torch.full((t_tensor.shape[0],1), 1000.0)).detach().to(device).requires_grad_(True)

# # lambda_p_im_phy      = torch.nn.Parameter(torch.tensor([1.0]), requires_grad=True).repeat(t_tensor.shape[0],1)
# # lambda_p_em_phy      = torch.nn.Parameter(torch.tensor([1.0]), requires_grad=True).repeat(t_tensor.shape[0],1)
# # lambda_omega_t_phy   = torch.nn.Parameter(torch.tensor([1.0]), requires_grad=True).repeat(t_tensor.shape[0],1)
# # lambda_u_egr1_phy    = torch.nn.Parameter(torch.tensor([1.0]), requires_grad=True).repeat(t_tensor.shape[0],1)
# # lambda_u_egr2_phy    = torch.nn.Parameter(torch.tensor([1.0]), requires_grad=True).repeat(t_tensor.shape[0],1)
# # lambda_u_vgt_bar_phy = torch.nn.Parameter(torch.tensor([1.0]), requires_grad=True).repeat(t_tensor.shape[0],1)
# # lambda_T_1_phy       = torch.nn.Parameter(torch.tensor([1.0]), requires_grad=True).repeat(t_tensor.shape[0],1)
# # lambda_x_r_phy       = torch.nn.Parameter(torch.tensor([1.0]), requires_grad=True).repeat(t_tensor.shape[0],1)

# # lambda_p_im_init      = torch.nn.Parameter(torch.tensor([1.0]), requires_grad=True)
# # lambda_p_em_init      = torch.nn.Parameter(torch.tensor([1.0]), requires_grad=True)
# # lambda_omega_t_init   = torch.nn.Parameter(torch.tensor([1.0]), requires_grad=True)
# # lambda_u_egr1_init    = torch.nn.Parameter(torch.tensor([1.0]), requires_grad=True)
# # lambda_u_egr2_init    = torch.nn.Parameter(torch.tensor([1.0]), requires_grad=True)
# # lambda_u_vgt_bar_init = torch.nn.Parameter(torch.tensor([1.0]), requires_grad=True)
# # lambda_T_1_init       = torch.nn.Parameter(torch.tensor([1.0]), requires_grad=True)
# # lambda_x_r_init       = torch.nn.Parameter(torch.tensor([1.0]), requires_grad=True)
...
#                             {'params': x_r_nn.parameters()}], lr=1e-3)
...
# # t_tensor mapping to [-1,1] from[0,30]

# t_tensor.requires_grad_(True)
# # t_tensor_norm_temp = time_mapping(t_tensor)

# # d_t_norm = grad(t_tensor_norm_temp, t_tensor, grad_outputs=torch.ones_like(t_tensor))
# # t_tensor_norm = t_tensor_norm_temp.detach()  ## detach and require_grad
# # t_tensor_norm.requires_grad_(True)
...
# Epoch = 20
# epoch_for_adp_weights = 10
# for epoch in range(Epoch):
    
#     # predict data
#     p_im_em_list_pred, A_egrmax_pred   = pim_pem_nn(t_tensor)
#     p_im_pred = p_im_em_list_pred[:,0].reshape(-1,1).requires_grad_(True)
#     p_em_pred = p_im_em_list_pred[:,1].reshape(-1,1).requires_grad_(True)
#     # transfer to phy domain
#     A_egrmax_pred_phy = A_egrmax_pred*1e-4
#     p_im_pred_phy = (softplus(p_im_pred)+1)*1e5
#     p_em_pred_phy = (softplus(p_em_pred)+1)*1e5


#     omega_t_pred, c_omega_psi_1_pred, c_omega_psi_2_pred, c_omega_psi_3_pred, \
#     c_omega_phi_1_pred, c_omega_phi_2_pred, c_omega_phi_3_pred                   = omega_t_nn(t_tensor)
#     # transfer to phy domain
#     omega_t_pred_phy = (softplus(omega_t_pred) + 0.7)* 5e3
#     c_omega_psi_1_pred_phy = c_omega_psi_1_pred * 1e-9
#     c_omega_psi_2_pred_phy = c_omega_psi_2_pred * 1e-3
#     c_omega_psi_3_pred_phy = c_omega_psi_3_pred * 1e-1
#     c_omega_phi_1_pred_phy = c_omega_phi_1_pred * 1e-7
#     c_omega_phi_2_pred_phy = c_omega_phi_2_pred * 1e-2
#     c_omega_phi_3_pred_phy = c_omega_phi_3_pred * 10

#     uegr_bar_list_pred  = uegr_1_2_nn(t_tensor)[0]
...
#     u_vgt_bar_pred     = u_vgt_nn(t_tensor)[0].reshape(-1,1)
#     T_1_pred           = T_1_nn(t_tensor)[0].reshape(-1,1)
#     x_r_pred           = x_r_nn(t_tensor)[0].reshape(-1,1)
#     # transfer to phy domain
#     u_vgt_bar_pred_phy = sigmoid(u_vgt_bar_pred) * 100
#     T_1_pred_phy = (softplus(T_1_pred)+0.5) * 300
#     x_r_pred_phy = (softplus(x_r_pred)+0.3)*0.03
...
#     T_em_physics, x_v_physics, T_e_physics, x_p_physics = Cylinder_ins.Cylinder_temperature(p_im_pred_phy, p_em_pred_phy, x_r_pred_phy, T_1_pred_phy, 
#                                                                                             W_eo_physics, W_ei_physics, W_f_physics)

#     W_egr_physics = EGR_ins.EGR_dynamics(p_im_pred_phy, p_em_pred_phy, uegr_1_bar_pred_phy, uegr_2_bar_pred_phy, T_em_physics, A_egrmax_pred_phy)

#     # used for calculate pem physics loss
#     Pt_eta_m_physics, W_t_physics = Turbo_ins.Turbo_charger_dynamics(omega_t_pred_phy, u_vgt_bar_pred_phy, p_em_pred_phy, T_em_physics)
...
#     loss3  = weighted_loss(lambda_omega_t_data , (omega_t_pred_phy - omega_t_gt))
#     loss4  = weighted_loss(1                   , p_im_physics_loss(p_im_pred_phy, t_tensor, R_a, T_im, V_im, W_c_physics, W_egr_physics, W_ei_physics) )
#     loss5  = weighted_loss(1                   , p_em_physics_loss(p_em_pred_phy, t_tensor, R_e, T_em_physics, V_em, W_eo_physics, W_egr_physics, W_t_physics)) 
#     loss6  = weighted_loss(1                   , omega_t_physics_loss(omega_t_pred_phy, t_tensor, Pt_eta_m_physics, p_c_physics, J_t))
#     loss7  = weighted_loss(1                   , u_egr1_physics_loss(uegr_1_bar_pred_phy, t_tensor, tau_egr1, u_egr, tau_degr))
#     loss8  = weighted_loss(1                   , u_egr2_physics_loss(uegr_2_bar_pred_phy, t_tensor, tau_egr2, u_egr, tau_degr))
#     loss9  = weighted_loss(1                   , u_vgt_bar_physics_loss(u_vgt_bar_pred_phy, t_tensor, tau_vgt, u_vgt, tau_dvgt))
#     loss10  = weighted_loss(10**3               , T_1_physics_loss(T_1_pred_phy, x_r_pred_phy, T_e_physics, T_im))
#     loss11 = weighted_loss(1                   , x_r_physics_loss(x_r_pred_phy, p_im_pred_phy, p_em_pred_phy, gamma_a, x_p_physics, r_c, x_v_physics))
#     loss12 = weighted_loss(1                   , (p_im_pred_phy[0] - p_im_Init))
#     loss13 = weighted_loss(1                   , (p_em_pred_phy[0] - p_em_Init))
#     loss14 = weighted_loss(1                   , (omega_t_pred_phy[0] - omega_t_Init)) 
#     loss15 = weighted_loss(1                   , (uegr_1_bar_pred_phy[0] - uegr_1_bar_Init)) 
#     loss16 = weighted_loss(1                   , (uegr_2_bar_pred_phy[0] - uegr_2_bar_Init))
#     loss17 = weighted_loss(1                   , (u_vgt_bar_pred_phy[0] - u_vgt_bar_Init)) 
#     loss18 = weighted_loss(10                  , (x_r_pred_phy[0] - x_r_Init)) 
#     loss19 = weighted_loss(100                 , (T_1_pred_phy[0] - T_1_Init))
...
#     LOSS19.append(loss19.detach().cpu().numpy())
...
#         # print(lambda_p_im_data)

# save_flag = 1
# ### plots generation
# LOSSES = [LOSS1, LOSS2, LOSS3, LOSS4, LOSS5, LOSS6, LOSS7, LOSS8, LOSS9, LOSS10,
#           LOSS11, LOSS12, LOSS13, LOSS14, LOSS15, LOSS16, LOSS17, LOSS18, LOSS19]
# # loss plot
...
#         plt.show()
#     else:
#         plt.savefig("pic_task2_2/Loss{}.png".format(i+1))
...
#     plt.show()
# else:
#     plt.savefig("pic_task2_2/Total_Loss.png")
...
# fig2, ax2 = plt.subplots(1,1,figsize=(25,5))
...
#     plt.show()
# else:
#     plt.savefig("pic_task2_2/Lambda1.png")
...
#     plt.show()
# else:
#     plt.savefig("pic_task2_2/Lambda2.png")


# plot_th = np.zeros((len(WATCH), WATCH[0].shape[0]))
# for i in range(len(WATCH)):
#     plot_th[i,:] = WATCH[i].reshape(1,WATCH[0].shape[0])
# x_time = np.arange(0,len(WATCH),1)
...
# ax2.tick_params(axis='y', labelsize=25)
# ax2.tick_params(axis='x', labelsize=25)
# ax2.set_title('WATCH', fontsize=30, pad=20)
# im = ax2.imshow(plot_th.T, cmap='jet', interpolation='nearest', aspect='auto')
# ax2.autoscale(False)
# cbar = fig2.colorbar(im, cax=axins)
# cbar.ax.tick_params(labelsize=20)
...
#     plt.show()
# else:
#     plt.savefig("pic_task2_2/WATCH.png")


# # unknown parameter plot
# plt.figure()
# plt.plot(xx, A_EGRMAX,label="Pred")
# plt.axhline(y=A_egrmax_gt,color='r',linestyle='--', label='GT')
# plt.legend()
# plt.title("A_EGRMAX Prediction")
# if save_flag == None:
#     plt.show()
# else:
#     plt.savefig("pic_task2_2/A_EGRMAX_Pred.png")


# plt.figure()
# plt.plot(xx, C_OMEGA_PSI_1,label="Pred")
# plt.axhline(y=c_wpsiVec_tensor[0],color='r',linestyle='--', label='GT')
# plt.title("C_OMEGA_PSI_1 Prediction")
# if save_flag == None:
#     plt.show()
# else:
#     plt.savefig("pic_task2_2/C_OMEGA_PSI_1_Pred.png")

# plt.figure()
# plt.plot(xx, C_OMEGA_PSI_2,label="Pred")
# plt.axhline(y=c_wpsiVec_tensor[1],color='r',linestyle='--', label='GT')
# plt.title("C_OMEGA_PSI_2 Prediction")
# if save_flag == None:
#     plt.show()
# else:
#     plt.savefig("pic_task2_2/C_OMEGA_PSI_2_Pred.png")

# plt.figure()
# plt.plot(xx, C_OMEGA_PSI_3,label="Pred")
# plt.axhline(y=c_wpsiVec_tensor[2],color='r',linestyle='--', label='GT')
# plt.title("C_OMEGA_PSI_3 Prediction")
# if save_flag == None:
#     plt.show()
# else:
#     plt.savefig("pic_task2_2/C_OMEGA_PSI_3_Pred.png")

# plt.figure()
# plt.plot(xx, C_OMEGA_PHI_1,label="Pred")
# plt.axhline(y=c_wphiVec_tensor[0],color='r',linestyle='--', label='GT')
# plt.title("C_OMEGA_PHI_1 Prediction")
# if save_flag == None:
#     plt.show()
# else:
#     plt.savefig("pic_task2_2/C_OMEGA_PHI_1_Pred.png")

# plt.figure()
# plt.plot(xx, C_OMEGA_PHI_2,label="Pred")
# plt.axhline(y=c_wphiVec_tensor[1],color='r',linestyle='--', label='GT')
# plt.title("C_OMEGA_PHI_2 Prediction")
# if save_flag == None:
#     plt.show()
# else:
#     plt.savefig("pic_task2_2/C_OMEGA_PHI_2_Pred.png")

# plt.figure()
# plt.plot(xx, C_OMEGA_PHI_3,label="Pred")
# plt.axhline(y=c_wphiVec_tensor[2],color='r',linestyle='--', label='GT')
# plt.title("C_OMEGA_PHI_3 Prediction")
# if save_flag == None:
#     plt.show()
# else:
#     plt.savefig("pic_task2_2/C_OMEGA_PHI_3_Pred.png")


# ## save model
# torch.save(pim_pem_nn.state_dict() , 'model_task2_2/pim_pem_nn.pth')
# torch.save(omega_t_nn.state_dict() , 'model_task2_2/omega_t_nn.pth')
# torch.save(uegr_1_2_nn.state_dict(), 'model_task2_2/uegr_1_2_nn.pth')
# torch.save(u_vgt_nn.state_dict()   , 'model_task2_2/u_vgt_nn.pth')
# torch.save(T_1_nn.state_dict()     , 'model_task2_2/T_1_nn.pth')
# torch.save(x_r_nn.state_dict()     , 'model_task2_2/x_r_nn.pth')

# ## test model on train set
# p_im_pred_phy_norm = normalize(p_im_pred_phy)
# p_im_tensor_norm   = normalize(p_im_tensor)
# plt.figure()
# plt.plot(t_tensor.detach().cpu().numpy(), p_im_pred_phy_norm.detach().cpu().numpy(), label="pred")
# plt.plot(t_tensor.detach().cpu().numpy(), p_im_tensor_norm.detach().cpu().numpy(), '--',label="GT")
# plt.title("p_im")
# plt.legend()
# if save_flag == None:
#     plt.show()
# else:
#     plt.savefig("pic_task2_2/p_im.png")

# p_em_pred_phy_norm = normalize(p_em_pred_phy)
# p_em_tensor_norm   = normalize(p_em_tensor)
# plt.figure()
# plt.plot(t_tensor.detach().cpu().numpy(), p_em_pred_phy_norm.detach().cpu().numpy(), label="pred")
# plt.plot(t_tensor.detach().cpu().numpy(), p_em_tensor_norm.detach().cpu().numpy(), '--',label="GT")
# plt.title("p_em")
# plt.legend()
# if save_flag == None:
#     plt.show()
# else:
#     plt.savefig("pic_task2_2/p_em.png")

# uegr_1_bar_pred_phy_norm = normalize(uegr_1_bar_pred_phy)
# uegr_1_bar_norm   = normalize(uegr_1_bar)
# plt.figure()
# plt.plot(t_tensor.detach().cpu().numpy(), uegr_1_bar_pred_phy_norm.detach().cpu().numpy(), label="pred")
# plt.plot(t_tensor.detach().cpu().numpy(), uegr_1_bar_norm.detach().cpu().numpy(), '--',label="GT")
# plt.title("u_egr_bar_1")
# plt.legend()
# if save_flag == None:
#     plt.show()
# else:
#     plt.savefig("pic_task2_2/u_egr_bar_1.png")

# uegr_2_bar_pred_phy_norm = normalize(uegr_2_bar_pred_phy)
# uegr_2_bar_norm   = normalize(uegr_2_bar)
# plt.figure()
# plt.plot(t_tensor.detach().cpu().numpy(), uegr_2_bar_pred_phy_norm.detach().cpu().numpy(), label="pred")
# plt.plot(t_tensor.detach().cpu().numpy(), uegr_2_bar_norm.detach().cpu().numpy(), '--',label="GT")
# plt.title("u_egr_bar_2")
# plt.legend()
# if save_flag == None:
#     plt.show()
# else:
#     plt.savefig("pic_task2_2/u_egr_bar_2.png")

# u_vgt_bar_pred_phy_norm = normalize(u_vgt_bar_pred_phy)
# u_vgt_bar_norm   = normalize(u_vgt_bar)
# plt.figure()
# plt.plot(t_tensor.detach().cpu().numpy(), u_vgt_bar_pred_phy_norm.detach().cpu().numpy(), label="pred")
# plt.plot(t_tensor.detach().cpu().numpy(), u_vgt_bar_norm.detach().cpu().numpy(), '--',label="GT")
# plt.title("u_vgt_bar")
# plt.legend()
# if save_flag == None:
#     plt.show()
# else:
#     plt.savefig("pic_task2_2/u_vgt_bar.png")

# omega_t_pred_phy_norm = normalize(omega_t_pred_phy)
# omega_t_norm   = normalize(omega_t_gt)
# plt.figure()
# plt.plot(t_tensor.detach().cpu().numpy(), omega_t_pred_phy_norm.detach().cpu().numpy(), label="pred")
# plt.plot(t_tensor.detach().cpu().numpy(), omega_t_norm.detach().cpu().numpy(), '--',label="GT")
# plt.title("omega_t")
# plt.legend()
# if save_flag == None:
#     plt.show()
# else:
#     plt.savefig("pic_task2_2/omega_t.png")

# T_1_pred_phy_norm = normalize(T_1_pred_phy)
# T_1_norm   = normalize(T_1)
# plt.figure()
# plt.plot(t_tensor.detach().cpu().numpy(), T_1_pred_phy_norm.detach().cpu().numpy(), label="pred")
# plt.plot(t_tensor.detach().cpu().numpy(), T_1_norm.detach().cpu().numpy(), '--',label="GT")
# plt.title("T_1")
# plt.legend()
# if save_flag == None:
#     plt.show()
# else:
#     plt.savefig("pic_task2_2/T_1.png")

# x_r_pred_phy_norm = normalize(x_r_pred_phy)
# x_r_norm   = normalize(x_r)
# plt.figure()
# plt.plot(t_tensor.detach().cpu().numpy(), x_r_pred_phy_norm.detach().cpu().numpy(), label="pred")
# plt.plot(t_tensor.detach().cpu().numpy(), x_r_norm.detach().cpu().numpy(), '--',label="GT")
# plt.title("x_r")
# plt.legend()
# if save_flag == None:
#     plt.show()
# else:
#     plt.savefig("pic_task2_2/x_r.png")
...
A_egrmax_train       = torch.nn.Parameter(torch.tensor([3.0]) ).detach().to(device).requires_grad_(True)
c_omega_psi_1_train  = torch.nn.Parameter(torch.tensor([1.0])).detach().to(device).requires_grad_(True)
c_omega_psi_2_train  = torch.nn.Parameter(torch.tensor([1.0])).detach().to(device).requires_grad_(True)
c_omega_psi_3_train  = torch.nn.Parameter(torch.tensor([1.0])).detach().to(device).requires_grad_(True)
c_omega_phi_1_train  = torch.nn.Parameter(torch.tensor([1.0])).detach().to(device).requires_grad_(True)
c_omega_phi_2_train  = torch.nn.Parameter(torch.tensor([1.0])).detach().to(device).requires_grad_(True)
c_omega_phi_3_train  = torch.nn.Parameter(torch.tensor([2.5])).detach().to(device).requires_grad_(True)
...
for epoch in range(Epoch):
    ...
    if epoch < epoch_for_adp_weights:  # train lambda
        ...
        # if A_egrmax_train >= 3.93:
        #     scheduler.step()  # redeuce lr to 1e-6
    ...
    if epoch % 1000 == 999:
        print('Epoch {}, Loss: {:.8e}'.format(epoch+1, loss.item()))
        print(A_egrmax_train)
        print(A_egrmax_train.grad)
        print(" ")
...
if save_flag == None:
    ...
    plt.savefig("pic_task2_2/Lambda3.png")
    plt.close()
