...
lambda_p_im_data     = torch.nn.Parameter(torch.full((t_tensor.shape[0],1), 10.0,device=device),requires_grad=True)
lambda_p_em_data     = torch.nn.Parameter(torch.full((t_tensor.shape[0],1), 1.0,device=device),requires_grad=True)
...
A_egrmax_train       = torch.nn.Parameter(torch.tensor([3.0],device=device),requires_grad=True)
...
# scheduler = MultiStepLR(optimizer, milestones=[1], gamma=1e-3)
...
for epoch in range(Epoch):
    ...
    loss5  = weighted_loss(1000                , omega_t_physics_loss(omega_t_pred_phy, t_tensor, Pt_eta_m_physics, p_c_physics, J_t))
    loss6  = weighted_loss(1                   , u_egr1_physics_loss(uegr_1_bar_pred_phy, t_tensor, tau_egr1, u_egr, tau_degr))
    loss7  = weighted_loss(10                   , u_egr2_physics_loss(uegr_2_bar_pred_phy, t_tensor, tau_egr2, u_egr, tau_degr))
